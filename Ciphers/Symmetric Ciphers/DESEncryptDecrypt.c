...
void keysReverser(char keys[ROUNDS+1][PERMUTEDLENGTH+1]) {
    char temporarySwapVariable[PERMUTEDLENGTH+1];
    if (ROUNDS%2==0) {
        for (int i=0; i < ROUNDS/2; i++) {
            strcpy(temporarySwapVariable, keys[i]);
            strcpy(keys[i], keys[ROUNDS-1-i]);
            strcpy(keys[ROUNDS-1-i], temporarySwapVariable);
        }
    }
}
